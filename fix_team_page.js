...
// Revert useAuthStore
content = content.replace(/const \{ user, setUser \} = useAuthStore\(\);/, 'const { user } = useAuthStore();');
...
content = content.replace(/\.in\('type', \['referral_bonus', 'team_bonus'\]\);/, `.eq('type', 'referral_bonus');`);

// Remove bonus UI block
content = content.replace(/<div className="mb-8">\n        <h2 className="text-sm font-black text-slate-900 mb-3 flex items-center gap-2">[\s\S]*?Bonus d'invitation[\s\S]*?<\/h2>[\s\S]*?<\/div>\n      <\/div>\n\n/, '');
