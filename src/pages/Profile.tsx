...
export function Profile() {
  ...
  const accountId = user?.referral_code 
    ? user.referral_code 
    : (user?.id ? `ORLEN-${user.id.slice(-6).toUpperCase()}` : 'ORLEN-1001');
    ...
    <div className="min-h-screen pb-28 font-sans text-slate-900 bg-slate-50 overflow-x-hidden">
      ...
      <header className="sticky top-0 z-30 bg-white/95 backdrop-blur-md border-b border-slate-200 px-4 py-3 flex justify-between items-center transition-all">
        <div>
          ...
          <p className="text-red-700 text-[10px] font-black uppercase tracking-wider">Espace Partenaire ORLEN</p>
        </div>
      ...
      <div className="pt-3 max-w-lg mx-auto space-y-4 px-3 sm:px-0">
        ...
        {depositSuccessMsg && (
          <div className="p-4 bg-red-50 border border-red-300 rounded-2xl text-red-900 text-xs font-bold flex items-center justify-between shadow-sm animate-in fade-in">
        ...
        <div className="bg-white border border-slate-200 rounded-2xl p-4 sm:p-5 flex items-center justify-between shadow-sm">
          <div className="space-y-1">
            ...
            <p className="text-xs font-semibold text-slate-600 flex items-center gap-1.5">
              <span className="text-slate-500 font-medium">Numéro de compte :</span>
              <span className="font-bold text-slate-900 font-mono">{accountNumber}</span>
            </p>
          </div>
          <div className="flex items-center gap-1.5 px-2.5 py-1 bg-red-50 text-red-800 text-xs font-black rounded-full border border-red-200 shrink-0">
        ...
        <div className="bg-white border border-slate-200 rounded-2xl p-5 shadow-sm space-y-4">
          ...
          <div className="grid grid-cols-2 gap-2.5 pt-1">
            <Link 
              to="/deposit" 
              className="bg-red-600 hover:bg-red-700 text-white transition-all py-3 px-4 rounded-xl flex items-center justify-center gap-2 font-black text-xs shadow-sm active:scale-98 cursor-pointer"
            >
            ...
            >
              <Banknote className="w-4 h-4 text-red-700" />
              <span>Retirer</span>
            </Link>
          </div>
        </div>
        ...
        <div className="space-y-2">
          ...
          <Link
            to="/withdraw-info"
            className="bg-white border border-slate-200 rounded-xl p-4 flex items-center justify-between shadow-sm hover:border-red-400 transition-all cursor-pointer"
          >
          ...
          >
            <div className="flex items-center gap-3.5">
              ...
              <div>
                ...
                <span className="text-slate-500 text-xs font-medium">Communauté Telegram ORLEN</span>
              </div>
            </div>
          ...
          <button
            ...
            className="w-full bg-white border border-slate-200 rounded-xl p-4 flex items-center justify-between text-left shadow-sm hover:border-red-400 transition-all cursor-pointer"
          >
            <div className="flex items-center gap-3.5">
            ...
            <span className="px-2.5 py-1 rounded-lg bg-red-50 text-red-800 text-[11px] font-black border border-red-200">
              PWA
            </span>
          </button>
          ...
          <div className="bg-white rounded-3xl max-w-sm w-full p-6 space-y-4 shadow-2xl border border-slate-200 animate-in fade-in zoom-in-95">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <h3 className="font-black text-slate-900 text-base">Installer ORLEN</h3>
              </div>
              <button
            ...
            <p className="text-xs text-slate-600 leading-relaxed font-medium">
              Profitez d’une expérience fluide et rapide en installant l’application ORLEN directement sur votre écran d’accueil :
            </p>
            ...
            <button
              onClick={() => setShowDownloadModal(false)}
              className="w-full py-3 rounded-xl bg-red-600 hover:bg-red-700 text-white font-black text-xs uppercase tracking-wider transition-all cursor-pointer"
            >
              J’ai compris
            </button>
          </div>
      ...
      <div className="fixed bottom-24 right-5 z-40">
        <button
          ...
          aria-label="Assistance Client ORLEN"
        >
          <div className="w-full h-full rounded-xl bg-red-50 flex items-center justify-center text-red-700">
