    ...
    <div className="p-6 space-y-6 pb-24 pt-20 max-w-lg mx-auto">
      ...
      {message && (
        <div className="fixed top-4 left-1/2 -translate-x-1/2 z-[100] w-[90%] max-w-md">
          <div className={`p-4 rounded-xl text-sm font-bold shadow-2xl flex items-center gap-3 ${message.type === 'error' ? 'bg-red-50 text-red-700 border-2 border-red-200' : 'bg-brand-50 text-brand-700 border-2 border-brand-200'}`}>
            {message.text}
          </div>
        </div>
      )}
                ...
                <div key={inv.id} className="bg-white border-slate-200/80 shadow-slate-200/50 border border-slate-200 rounded-2xl p-4 shadow-sm relative overflow-hidden">
                  ...
                  <div className="flex justify-between items-start mb-2 pl-2">
                    ...
                    <div className="flex flex-col items-end gap-2">
                      ...
                      <button onClick={() => handleRemoveInvestment(inv.id)} disabled={loading} className="text-brand-600 hover:bg-brand-600/10 p-1.5 rounded-lg transition-colors" title="Supprimer l'investissement">
              ...
              <div key={u.id} className="bg-white border-slate-200/80 shadow-slate-200/50 border border-slate-200 rounded-2xl p-4 shadow-sm relative">
                ...
                {editingUserId === u.id ? (
                  ...
                  <div className="flex gap-2 mt-3 pt-3 border-t border-slate-200">
                    ...
                    {u.role !== 'admin' && (
                      <button onClick={() => handleDeleteUser(u.id)} className="p-2 bg-brand-600/10 text-brand-600 border border-brand-600/20 rounded-xl hover:bg-brand-600/20 transition-colors cursor-pointer">
                        <Trash2 className="w-4 h-4" />
                      </button>
                    )}
                  </div>
              ...
              <div key={tx.id} className="bg-white border-slate-200/80 shadow-slate-200/50 border border-slate-200 rounded-2xl p-4 shadow-sm">
                <div className="flex justify-between items-start mb-2">
                  ...
                  <div className={`text-[10px] font-bold px-2 py-1 rounded-md uppercase tracking-wider ${
                    ...
                    tx.status === 'approved' ? 'bg-brand-600/10 text-brand-800 border border-brand-600/20' :
                ...
                {tx.status === 'pending' && (
                  <div className="flex gap-2 mt-4 pt-4 border-t border-slate-200">
                    ...
                    <button onClick={() => handleTransaction(tx.id, 'rejected', tx.type, tx.amount, tx.user_id)} className="flex-1 bg-brand-600/10 hover:bg-brand-600/20 text-brand-700 py-2 rounded-xl flex items-center justify-center gap-2 text-sm font-medium transition-colors cursor-pointer">
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-2 mt-1 mb-2">
                      ...
                      <div className="bg-brand-600/10 px-2 py-0.5 rounded-md border border-brand-600/20">
                        <p className="text-[9px] text-brand-600/70 uppercase tracking-wider">Montant à envoyer</p>
                        <p className="text-brand-500 font-bold text-xs">{formatCurrency(tx.amount * 0.90)}</p>
                      </div>
                    </div>
                    {(() => {
            ...
            {isInitializing ? (
              ...
              <div key={idx} className="flex items-center justify-between p-4 bg-white border-slate-200/80 shadow-slate-200/50 rounded-2xl border border-slate-200 shadow-sm relative overflow-hidden">
                ...
                <div className="flex items-center gap-2">
                  <button onClick={() => handleEditPlan(idx)} disabled={loading} className="p-2.5 text-brand-600 bg-brand-600/10 border border-brand-600/20 rounded-xl hover:bg-brand-600/20 transition-colors cursor-pointer">
                    <Edit className="w-5 h-5" />
                  </button>
                  <button onClick={() => handleRemovePlan(idx)} disabled={loading} className="p-2.5 text-brand-600 bg-brand-600/10 border border-brand-600/20 rounded-xl hover:bg-brand-600/20 transition-colors cursor-pointer">
          ...
          <div className="bg-white border-slate-200/80 shadow-slate-200/50 border border-slate-200 rounded-3xl p-6 shadow-sm">
            ...
            <div className="space-y-4">
              ...
              <div className="pt-6 mt-6 border-t border-brand-600/20">
                <h3 className="text-brand-600 font-bold mb-3">Zone de Danger</h3>
                <button 
