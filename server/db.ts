    ...
    try {
      ...
      if (fs.existsSync(DB_FILE)) {
        ...
        if (parsed && Array.isArray(parsed.users)) {
          // Filtrer les comptes de démonstration et supprimer tous les comptes non-administrateurs demandés
          const cleanedUsers = parsed.users.filter((u: UserRecord) => 
          ...
          const cleanedInvestments = (Array.isArray(parsed.investments) ? parsed.investments : []).filter(
            (i: InvestmentRecord) => !i.id.startsWith('inv-team-') && !i.user_id.startsWith('usr-team-')
          );
