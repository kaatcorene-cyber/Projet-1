...
if (match) {
        ...
        <form onSubmit={handleSubmit} className="space-y-6 relative z-10">
           ...
           {error && (
            <motion.div 
              initial={{ opacity: 0, scale: 0.95 }}
              animate={{ opacity: 1, scale: 1 }}
              className="p-4 bg-red-50 text-red-600 rounded-2xl mb-6 text-sm font-semibold flex items-center gap-2 border border-red-100 shadow-sm"
            >
  ...
  // Also we need to remove the extra </motion.div> at the end of the file since we removed the wrapping motion.div
  ...
  const newSubmit = `<div className="pt-2">
                 ...
                 >
                   {loading ? 'Redirection...' : 'Confirmer le retrait'}
                 </button>
               </div>`;
